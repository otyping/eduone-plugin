...
def in_work_root() -> bool:
    """ยืนอยู่ในโฟลเดอร์งานจริงไหม — หมุดคือ CLAUDE.md หรือ Output/ (ตรงกับ _root.py)

    ★ ความผิดพลาดที่พบบ่อยที่สุดของหน้านี้คือรันจากโฟลเดอร์บ้าน แล้วมันไม่ฟ้องเอง:
      _root.find_work_root() ไม่เจอหมุดก็ตกกลับมาใช้ cwd ตามที่ออกแบบไว้ หน้าจอจึง
      ขึ้นครบทุกบรรทัดแต่ว่างหมด อ่านได้ว่า "งานยังไม่เริ่ม" ทั้งที่ความจริงคือ
      กำลังส่องผิดที่ · แย่กว่านั้นคือมันรายงาน "0 ไฟล์" ขึ้นเว็บทับของจริงด้วย
      (เกิดจริง 2026-09-03 กับใบสั่ง #2 — หน้าเว็บค้างที่ 0/7 ทั้งที่ไฟล์ครบแล้ว)
    """
    return (WORK_ROOT / "CLAUDE.md").is_file() or (WORK_ROOT / "Output").is_dir()
...
def main() -> int:
    ...
    if not in_work_root():
        print(c("red", "  ✗ ที่นี่ไม่ใช่โฟลเดอร์งาน") + f" — {WORK_ROOT}")
        print("    หน้านี้อ่านไฟล์จากโฟลเดอร์ที่คุณยืนอยู่ ยืนผิดที่จะเห็นทุกช่องว่าง")
        print("    เหมือนงานยังไม่เริ่ม ทั้งที่งานอาจทำเสร็จไปแล้วในอีกโฟลเดอร์หนึ่ง")
        print(c("dim", "\n    cd เข้าโฟลเดอร์งานก่อน (โฟลเดอร์ที่มี CLAUDE.md กับ Output/) แล้วรันใหม่"))
        print(c("dim", "    เช่น  cd C:\\Users\\<ชื่อคุณ>\\Claude\\EDUONE-MCP-RAG\n"))
        return 2
