...
if (-not $haveCfg) {
    if ($tokenBad) {
        ...
        # ★ ห้ามขึ้น "ยังไม่ได้ตั้ง" ต่อจากบรรทัดที่เพิ่งบอกว่า "ตั้งไว้ผิด" - ขัดกันเอง
        #   และพนักงานจะไปหาโทเคนใบใหม่ทั้งที่ใบเดิมยังดีอยู่ ทั้งสองปุ่มกด Enter ผ่านได้เลย
        Say "  โทเคนใบเดิมยังอยู่ ไม่ต้องไปออกใหม่ - กด Enter ผ่านได้ทั้งสองช่องข้างล่าง"
    } else {
...
#   รุ่นเก่าที่ไม่มี runner.py แล้วทางลัดที่เพิ่งตั้งจะเปิดหน้าต่างที่ฟ้องว่าหาสคริปต์ไม่เจอ
#   ทุกครั้งที่เปิดเครื่อง โดยที่เว็บก็ยังขึ้น "รอเครื่องของคุณออนไลน์" เหมือนเดิม
#   - พังสองชั้นโดยไม่มีใครรู้ว่าต้นเหตุอยู่ที่การตอบ N เมื่อกี้
$plug = Get-PluginDir
$haveRunner = $plug -and (Test-Path (Join-Path $plug "scripts\runner.py"))
if (-not $haveRunner) {
    Bad "ปลั๊กอินที่ติดตั้งอยู่ยังไม่มีตัวรับงาน (ต้อง 3.2.0 ขึ้นไป)"
    Say "  รันตัวช่วยนี้ซ้ำอีกรอบ แล้วตอบ Y ที่ข้อ 4 เพื่ออัปเดตปลั๊กอินก่อน"
    $script:todo += "อัปเดตปลั๊กอินให้มีตัวรับงาน"
} elseif (Ask "  ตั้ง/อัปเดตให้ตัวรับงานเริ่มเองตอนเปิดเครื่องไหม") {
