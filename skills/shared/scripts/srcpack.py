...
def render(data, src_name):
    ...
    for label in ("รหัสวิชา", "วิชา", "หน่วย", "ตัวชี้วัด", "เรื่อง"):
        vals = _rows(cover, label)
        if vals:
            add("- **%s:** %s" % (label, " / ".join(vals)))
    add("")

    # ป้ายชื่อต้องตรงกับ COVER_LABELS ใน validate_spec.py — ถ้าไม่ตรง srcpack จะว่าง
    # โดยไม่มีใครร้อง แล้วงานปลายน้ำทุกชิ้นจะเขียนโดยไม่รู้จุดประสงค์
    obj = _rows(cover, "จุดประสงค์ประจำหน่วย")
    comp = _rows(cover, "สาระสำคัญ / จุดประสงค์ประจำคาบ")
    if obj:
        add("## จุดประสงค์ประจำหน่วย (OBJ — ระดับหน่วย ไม่ใช่ของคาบนี้ทั้งหมด)")
        add(_bullets(obj))
        add("")
    if comp:
        add("## ★ สาระสำคัญ / จุดประสงค์ประจำคาบ (COMP — สิ่งที่คาบนี้ต้องส่งมอบ)")
        add(_bullets(comp))
        add("")
