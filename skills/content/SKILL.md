...
### (b) เตรียม cover.rows (7 แถว) จาก metadata
ใช้กับทั้ง C1 และ C2 (เหมือนกัน):
```
["รหัสวิชา", subject_code]
["วิชา", subject]
["หน่วย", unit_name]
["ตัวชี้วัด", indicator[]]                        # = indicator จาก metadata (คอลัมน์ F)
["เรื่อง", topic_name]
["จุดประสงค์ประจำหน่วย", obj[]]                    # = obj จาก metadata (คอลัมน์ E ระดับหน่วย)
["สาระสำคัญ / จุดประสงค์ประจำคาบ", comp[]]         # = comp จาก metadata (คอลัมน์ I ระดับคาบ)
```
- `cover.title` = `"เอกสารประกอบเนื้อหา"`
- `cover.grade_line` = `"ระดับชั้น" + grade` (เช่น `"ระดับชั้นประถมศึกษาปีที่ 1"`)
- **ถ้า `obj[]` หรือ `comp[]` ว่าง** → สั่งให้ writer ร่างเองจากหัวข้อ + ระดับชั้น
  (`comp_source` ใน metadata บอกที่มา: `period` = ของจริงรายคาบ · `unit_outcome` = ของสำรองระดับหน่วย · `none` = ไม่มีเลย)
- **ถ้า `indicator[]` ว่าง** → ใส่ `"—"` ในแถวตัวชี้วัด ห้ามแต่งรหัสตัวชี้วัดขึ้นเอง
...
```
ตรวจ: หน้าปก 7 แถวเรียงถูก · `mode_label` · ชนิด block · ตารางคอลัมน์เท่ากัน ·
...
- **หน้าปก** (หน้า 1): หัวเรื่อง 18pt + บรรทัดระดับชั้น 18pt + ตาราง **7 แถว** แล้ว page break
