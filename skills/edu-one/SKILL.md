...
## ★ Preflight — เริ่มงานผลิตในเซสชันใหม่เสมอ

ก่อนทำอะไร ให้ดูว่าเซสชันนี้เพิ่งเปิดใหม่หรือทำงานอื่นมาก่อน
**ถ้าเซสชันนี้ทำงานอื่นมาก่อนหน้า (เขียนเว็บ · แก้สคริปต์ · ผลิตคาบอื่น) ให้หยุดแล้วบอกผู้ใช้ว่า**

> งานผลิตสื่อควรเริ่มจากเซสชันใหม่ — พิมพ์ `/clear` แล้วสั่งใหม่
> หรือรันผ่าน `scripts/run_period.ps1 <grade> <subject> <no>` ซึ่งแยกโปรเซสให้เอง

ทำไม (ตัวเลขจริงจากการวัด 1 คาบเต็ม)
- ต้นทุน 1 คาบ = **$308** โดย **orchestrator กินไปเอง 54% ($165.92)**
- เกือบทั้งหมดเป็น cache read — บริบทของแม่เฉลี่ย **726,000 โทเคน ถูกอ่านซ้ำ 316 turn**
- และแม่ **เริ่ม pipeline ที่ 585,680 โทเคนตั้งแต่ turn แรก** เพราะเซสชันนั้นทำงานเว็บ
  มาทั้งวันก่อนหน้า — ทุกอย่างที่อ่านไว้ตอนนั้นยังค้างอยู่และถูกจ่ายเงินอ่านใหม่ทุก turn
- เริ่มเซสชันใหม่ แม่จะเริ่มที่ ~30K แทน 585K = **ประหยัด ~$80-100 ต่อคาบ
  โดยไม่แตะโมเดลและไม่แตะคุณภาพ**

**ผลิตหลายคาบ = หนึ่งคาบหนึ่งเซสชัน** ไม่ใช่ 120 คาบในเซสชันเดียว
(บริบทแม่ชน 1M แล้ว compact ไปแล้วหนึ่งครั้งตั้งแต่คาบเดียว)
...
| 5 | video | `/video` | วิดีโอ (script/asset; render เป็น stub) |  ← **พักไว้ ห้ามผลิต**
| 6 | exercise | `/exercise` | แบบฝึกหัดปรนัย 30 ข้อ + เฉลย + สื่อรูป/เสียง ({BASE}_ex.json ไฟล์เดียว + .docx) |
| 7 | game | `/game` | คลังคำถามเกม Kahoot game.json (สุ่มเล่นครั้งละ 10 ข้อ) |  ← **พักไว้ ห้ามผลิต**

## ★ ผลผลิตที่ถูกพักไว้ — ห้ามผลิตจนกว่าเจ้าของโปรเจกต์จะสั่งเปิด

| ผลผลิต | สถานะ | เหตุผล |
|---|---|---|
| 5 video | **พักไว้** | ยังไม่พัฒนาตัวเรนเดอร์ |
| 7 game | **พักไว้** | ยังไม่พัฒนาต่อจาก game-app |

**ผู้ใช้สั่ง "ทำทั้งหมด" ก็ยังไม่ผลิตสองตัวนี้** — ให้ข้ามไป แล้วบอกในรายงานว่า
"พักไว้ตามที่เจ้าของโปรเจกต์กำหนด" ถ้าผู้ใช้สั่งชื่อผลผลิตนี้มาตรง ๆ ให้ถามยืนยันก่อน
อย่าเดาว่าเขาลืม

รายชื่อนี้ต้องตรงกับ `DEFERRED_PRODUCTS` ใน `webapp/app/catalog.py` ของ repo งาน
ซึ่งเป็นตัวที่กันไม่ให้สั่งผ่านหน้าเว็บ — **แก้ที่ไหนต้องแก้อีกที่ด้วย** ไม่งั้นคนที่รัน
`/edu-one` ตรง ๆ ในเครื่องจะได้ผลต่างจากคนที่สั่งผ่านเว็บ (เคยเกิดจริง: ผลิตวิดีโอกับเกม
ไปแล้วทั้งที่เจ้าของโปรเจกต์สั่งพักไว้ เพราะข้อมูลอยู่ในเว็บที่เดียว)
...
- **ผลผลิตที่เปิดใช้อยู่ ผลิตครบทุกครั้ง** — C2 และ L2 เป็นไฟล์ส่งงานเช่นกัน
  **ห้ามตัดออกหรือรอให้สั่งเพิ่ม** เช่นเดียวกับสไลด์ที่ต้องครบทั้ง 4 แหล่ง
- **★ แต่ผลผลิตที่ถูกพักไว้ ห้ามผลิต** แม้ผู้ใช้จะสั่ง "ทั้งหมด" — ดูหัวข้อถัดไป
...
5. **★ ผลของ checkwork ก็ส่งเป็น path เหมือนกัน** — checkwork เขียนรายการแก้ลง
   `review_md` (`{BASE}_gate_<tag>_review.md`) แล้วคืนมาแค่ `VERDICT` + สรุปหนึ่งบรรทัด
   orchestrator **ส่ง path นั้นต่อให้ writer ไปอ่านเอง ห้ามคัดรายการแก้มาพิมพ์ลง prompt**
   เหตุผลเดียวกับกฎข้อ 2 และ 3: รายการแก้ที่แม่พิมพ์เองจะค้างในบริบทของแม่ตลอด pipeline
   และตกหล่นเมื่อไร ลูกก็แก้ไม่ครบตามนั้นทันที (รอบ M1-Math_U1_1_2 เกิดการคัดลอกแบบนี้ 7 ครั้ง)
